 ...
 */
public class StateCodeListView extends AbstractView {
	/**
	 * 
	 */
	private static final long serialVersionUID = 552420595333207494L;
	...
	private ActionSet
		actionEdit = new ActionSet("EDIT_STATE_CODE", true);
	...
	private class Listener implements STableSelectionListener, STableEditorListener, ActionListener {
        ...
        public void invokeEditor(STableEditorEvent e) {
			StateCodeListModel model = getStateCodeListModel();
			if (model == null) {
				return;
			}
	 ...
	 */
	public StateCodeListView(Model model) {
		this();
		setModel(model);
	}
	...
	public boolean doEdit(int row) {
        ...
        fireUserEvent(EV_EDIT, getStateCodeListModel().getStateCode(row));	
        return true;
	}
	...
	public boolean doDelete(int row) {
        ...
        if (getStateCodeListModel() != null) {
        	success = getStateCodeListModel().doDeleteStateCode(row);
        }        
	...
	public boolean doOpen(int row) {
		fireUserEvent(EV_OPEN, getStateCodeListModel().getStateCode(row));	
		return true;
	}

	public boolean doNew() {
		fireUserEvent(EV_NEW, getStateCodeListModel());	
		return true;
	}
    ...
    */
	 ...
	 * f
	 */
	public StateCodeListModel getStateCodeListModel() {
		return getModel() instanceof StateCodeListModel ? (StateCodeListModel)getModel() : null;
	}
    ...
    public void setModel(Model newModel) {
	    if (!(newModel instanceof StateCodeListModel)) {
	        throw new IllegalArgumentException("Model not instance of StateCodeListModel");
	    }
