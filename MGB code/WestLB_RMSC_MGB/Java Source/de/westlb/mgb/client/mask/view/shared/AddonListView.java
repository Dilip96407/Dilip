 ...
 */
public class AddonListView extends AbstractView {
	/**
     * 
     */
    private static final long serialVersionUID = 3386336587079975091L;
	...
	private class Listener implements STableSelectionListener, STableEditorListener, ActionListener {
        ...
        public void invokeEditor(STableEditorEvent e) {
			AddonListModel model = getAddonListModel();
			if (model == null) {
				return;
			}
	 ...
	 */
	public AddonListView(Model model) {
		this();
		setModel(model);
	}
	...
	public boolean doEdit(int row) {
        ...
        fireUserEvent(EV_EDIT, getAddonListModel().getId(row));	
        return true;
	}
	...
	public boolean doDelete(int row) {
        ...
        if (getAddonListModel() != null) {
        	success = getAddonListModel().doDelete(row);
        }        
	...
	public boolean doOpen(int row) {
        ...
        fireUserEvent(EV_OPEN, getAddonListModel().getId(row));	
        return true;
	}

	public boolean doNew() {
		fireUserEvent(EV_NEW, getAddonListModel());	
		return true;
	}
    ...
    */
	 ...
	 * 
	 */
	public AddonListModel getAddonListModel() {
		return getModel() instanceof AddonListModel ? (AddonListModel)getModel() : null;
	}
    ...
    public void setModel(Model newModel) {
	    if (!(newModel instanceof AddonListModel)) {
	        throw new IllegalArgumentException("Model not instance of AddonListModel");
	    }
