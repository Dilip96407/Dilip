...
public class MoneyMarketTradeSheetBuilder extends TradeSheetBuilder {
		
	private static int i = 0; 
    private static final int COL_TRADE_TYPE = i++;
    private static final int COL_TRADE = i++;
    private static final int COL_TRADE_GROUP = i++;
	private static final int COL_INSTRUMENT = i++;
    private static final int COL_TRADER = i++;
	private static final int COL_PORTFOLIO = i++;
	...
	public MoneyMarketTradeSheetBuilder(Workbook workbook, String name) {
	    super(workbook, name);
	}
...
//      logger.debug("Create header row " + curRow);
        ...
        createHeaderCell(row, COL_TRADE_PRICE, "Trade price");
        int numberOfMarketPrices = NUMBER_OF_MARKET_PRICES;
        if (withMarketPrices) {
            createHeaderCell(row, COL_MARKET_PRICE, "Market price");
        ...
        createHeaderCell(row, COL_TURNOVER-numberOfMarketPrices, "Turnover");
...
//		logger.debug("Create trade row " + curRow + " for trade "+ trade.getTradeId());
        ...
        createCell(row, COL_TRADE_TYPE, trade.getTradeType());
        createCell(row, COL_TRADE, trade.getTradeId());
        createCell(row, COL_TRADE_GROUP, trade.getStornoGroupId());
        createCell(row, COL_INSTRUMENT, trade.getInstrument());
        createCell(row, COL_TRADER, trade.getTraderId());
        createCell(row, COL_PORTFOLIO, trade.getBookId());
        ...
        if (withMarketPrices) {
            if (trade.getCurrentPrice() != null) {
                createCell(row, COL_MARKET_PRICE, trade.getCurrentPrice().getMinPrice());
            }
            numberOfMarketPrices = 0;
        }
        ...
        createCell(row, COL_TURNOVER-numberOfMarketPrices, trade.getTurnover());
