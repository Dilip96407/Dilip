...
public class EquityTradeSheetBuilder extends TradeSheetBuilder {
	...
	private static final int COL_SETTLEMENT_DATE = i++;
	...
	public EquityTradeSheetBuilder(Workbook workbook, String name) {
	    super(workbook, name);
	}
...
//      logger.debug("Create header row " + curRow);
        ...
        createHeaderCell(row, COL_SETTLEMENT_DATE, "Settlement date");
...
//		logger.debug("Create trade row " + curRow + " for trade "+ trade.getTradeId());
        ...
        createCell(row, COL_SETTLEMENT_DATE, trade.getSettlementDate());
