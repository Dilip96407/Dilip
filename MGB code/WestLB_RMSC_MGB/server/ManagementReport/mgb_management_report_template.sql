...
WITH trade_count AS
     (SELECT   COUNT (t02_id) AS all_trades,
          ...
          FROM (SELECT t02_id,
                  ...
                  JOIN t08_job ON fk_t02_t08_job = t08_id
                     AND TRUNC (t08_creation_date) BETWEEN &start_date AND &stop_date
                     AND fk_t08_t09_mandant <> 'GLB'
     ...
     (SELECT   SUM (open_older_3) AS open_jobs_older_3,
          ...
          FROM (SELECT CASE
                          WHEN TRUNC (SYSDATE - t08_creation_date) > 7
                             THEN 1
                          ELSE 0
                       END AS open_older_7,
                       CASE
                          WHEN TRUNC (SYSDATE - t08_creation_date) > 3
                             THEN 1
                          ELSE 0
                       END AS open_older_3,
                       fk_t08_t09_mandant AS client
...
SELECT t09_name AS "client",
 ...
 FROM t09_mandant
 LEFT OUTER JOIN trade_count t ON t09_id = t.client
